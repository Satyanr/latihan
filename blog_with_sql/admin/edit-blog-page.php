...
try {
 ...
 $pageSlug = slug($pageTitle);
    ...
    $stmt = $db->prepare('UPDATE tbl_pages SET pageTitle = :pageTitle, pageSlug = :pageSlug, pageDescrip = :pageDescrip, pageContent = :pageContent, pageKeywords = :pageKeywords WHERE pageId = :pageId') ;
$stmt->execute(array(
    ':pageTitle' => $pageTitle,
    ':pageSlug' => $pageSlug,
    ...
    if(isset($error)){
        ...
        try {
           ...
           $stmt = $db->prepare('SELECT pageId, pageSlug,pageTitle, pageDescrip, pageContent, pageKeywords FROM tbl_pages WHERE pageId = :pageId') ;
